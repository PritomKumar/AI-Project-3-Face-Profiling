...
while True:
    ...
    for (x,y,w,h) in faces_detected:
        ...
        roi_gray2 = gray_img[y:y + w, x:x + h]
        ...
        id, conf = recognizer.predict(roi_gray2)
        ...
        if conf < 75 and conf > 45:
            ...
            print (labels[id])
            name = labels[id]
            cv2.putText(test_img, name, ((int(x-20), int(y - 5))), cv2.FONT_HERSHEY_SIMPLEX, .7, (255,255,255), 1, cv2.LINE_AA)
        ...
        cv2.putText(test_img, predicted_emotion, (int(x+w-50), int(y -5)), cv2.FONT_HERSHEY_SIMPLEX, .9, color, 1)
